...
const SslCertCard = (props: { data: any, title: string, actionButtons: any }): JSX.Element => {
  ...
  console.log(sslCert)
  const { version, subject, subjectAltName, issuer, fingerprint, serialNumber, asn1Curve, nistCurve, notAfter, notBefore, ext_key_usage, OCSP, caIssuers, crlDistributionPoints } = sslCert;
  return (
    <Card heading={props.title} actionButtons={props.actionButtons}>
      { version && <DataRow lbl="Version" val={version} /> }
      { subject && <DataRow lbl="Subject" val={subject[0][0][1]} /> }
      ...
      { notBefore && <DataRow lbl="Not Before" val={formatDate(notBefore)} /> }
      { notAfter && <DataRow lbl="Not  After" val={formatDate(notAfter)} /> }
      { serialNumber && <DataRow lbl="Serial Num" val={serialNumber} /> }
      { fingerprint && <DataRow lbl="Fingerprint" val={fingerprint} /> }
      { subjectAltName && <ListRow title="SubjectAltName" list={subjectAltName?.map((s) => `${s[0]}:${s[1]}`)} /> }
      { ext_key_usage && <ListRow title="Extended Key Usage" list={getExtendedKeyUsage(ext_key_usage)} /> }
      { OCSP && <ListRow title="OCSP" list={OCSP} /> }
      { caIssuers && <ListRow title="CA Issuers" list={caIssuers} /> }
      { crlDistributionPoints && <ListRow title="CRL DistributionPoints" list={crlDistributionPoints} /> }
