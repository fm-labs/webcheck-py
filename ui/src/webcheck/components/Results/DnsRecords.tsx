...
import Row, {ListRow} from '@/webcheck/components/Form/Row.tsx';
...
const TxtRecords = (props: { data: any }): JSX.Element => {
    const txtRecords = props.data;
    return (
        <>
            <h4>Parsed TXT records</h4>
            {!txtRecords && <Row lbl="" val="No TXT Records"/>}
            {Object.keys(txtRecords).map((recordName: string, index: number) => {
                return (
                    <Row lbl={recordName} val={txtRecords[recordName]} key={`${recordName}-${index}`}/>
                );
            })}
        </>
    );
...
const DnsRecordsCard = (props: { data: any, title: string, actionButtons: any }): JSX.Element => {
    //const dnsData = props.data;
    console.log("DNS Data:", props.data);
    const dnsRecords = props.data?.records;
    const txtRecords = props.data?.txt_records;
    if (!dnsRecords) {
        return (
    ...
    return (
        <Card heading={props.title} actionButtons={props.actionButtons} styles={styles}>
            <div className="content">
                ...
                {txtRecords && <TxtRecords data={txtRecords}/>}
            </div>
        </Card>
    );
}
