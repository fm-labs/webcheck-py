...
//import type { LoadingState } from '../components/misc/ProgressBar';
import type { AddressType } from '../utils/address-type-checker.ts';
import keys from '../utils/get-keys.ts';
...
const useMotherOfAllHooks = <ResultType = any>(params: UseIpAddressProps<ResultType>): ReturnType => {
  // Destructure params
  const { addressInfo, fetchRequest, jobId } = params;
  ...
  // Fire off the HTTP fetch request, then set results and update loading / error state
    ...
    // Function to update loading jobs - moved here to avoid circular dependency
    const updateLoadingJobs = (job: string | string[], newState: string, error?: string, retry?: (data?: any) => void | null, data?: any) => {
        // This is a stub function. Replace with actual implementation to update loading jobs in parent component.
        console.log(`Job: ${job}, State: ${newState}, Error: ${error}, Retry: ${retry ? 'callback defined' : 'callback not defined'}, Data: ${data}`);
    };
  ...
  const doTheFetch = () => {
    ...
    .then((res: any) => {
      if (!res) { // No response :(
        ...
        setResult({ error: errorMessage } as any);
